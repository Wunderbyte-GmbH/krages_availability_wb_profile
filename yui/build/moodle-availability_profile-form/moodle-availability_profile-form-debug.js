YUI.add('moodle-availability_wb_profile-form', function (Y, NAME) {

/**
 * JavaScript for form editing profile conditions.
 *
 * @module moodle-availability_wb_profile-form
 */
M.availability_wb_profile = M.availability_wb_profile || {};

/**
 * @class M.availability_wb_profile.form
 * @extends M.core_availability.plugin
 */
M.availability_wb_profile.form = Y.Object(M.core_availability.plugin);
 ...
 * @type Array
 */
M.availability_wb_profile.form.profiles = null;
 ...
 */
M.availability_wb_profile.form.initInner = function(standardFields, customFields) {
    this.standardFields = standardFields;
    this.customFields = customFields;
};

M.availability_wb_profile.form.getNode = function(json) {
    // Create HTML structure.
    var html = '<span class="availability-group"><label><span class="p-r-1">' +
            M.util.get_string('conditiontitle', 'availability_wb_profile') + '</span> ' +
    ...
    html += '</select></label> <label><span class="accesshide">' + M.util.get_string('label_operator', 'availability_wb_profile') +
    ...
    var operators = ['isequalto', 'contains', 'doesnotcontain', 'startswith', 'endswith',
            'isempty', 'isnotempty','isinlist'];
    for (i = 0; i < operators.length; i++) {
        html += '<option value="' + operators[i] + '">' +
                M.util.get_string('op_' + operators[i], 'availability_wb_profile') + '</option>';
    }
    html += '</select></label> <label><span class="accesshide">' + M.util.get_string('label_value', 'availability_wb_profile') +
            '</span><input name="value" type="text" class="form-control" style="width: 10em" title="' +
            M.util.get_string('label_value', 'availability_wb_profile') + '"/></label></span>';
    ...
    if (!M.availability_wb_profile.form.addedEvents) {
        M.availability_wb_profile.form.addedEvents = true;
        var updateForm = function(input) {
            var ancestorNode = input.ancestor('span.availability_wb_profile');
        ...
        root.delegate('change', function() {
             updateForm(this);
        }, '.availability_wb_profile select');
        root.delegate('change', function() {
             updateForm(this);
        }, '.availability_wb_profile input[name=value]');
    }

    return node;
};

M.availability_wb_profile.form.fillValue = function(value, node) {
    // Set field.
...
M.availability_wb_profile.form.fillErrors = function(errors, node) {
    var value = {};
    this.fillValue(value, node);

    // Check profile item id.
    if (value.sf === undefined && value.cf === undefined) {
        errors.push('availability_wb_profile:error_selectfield');
    }
    if (value.v !== undefined && /^\s*$/.test(value.v)) {
        errors.push('availability_wb_profile:error_setvalue');
    }
};
